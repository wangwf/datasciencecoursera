...
data$eventConvection <- data[grepl(regexLightning, data$EVTYPE, ignore.case = TRUE),]
...
bgn_date <- strsplit(as.character(D$bgn_date), "[^[:digit:]]")
bgn_date <- as.numeric(unlist(bgn_date))
...
There are 7 categories: Convection, Extreme Temperature, Flood, Marine, Tropical Cyclones, Winter and Other. In each category has many even types.
